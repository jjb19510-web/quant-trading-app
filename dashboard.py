...
def render_dashboard():
    ...
    st.write(indices)  # ?”ë²„ê¹?    if indices:
        row1 = indices[:3]
        row2 = indices[3:]
    ...
    if st.session_state.watchlist:
        ...
        with st.spinner("ê´€?¬ì¢…ëª??˜ìµë¥?ì¡°íšŒ ì¤?.."):
            wl_data = get_watchlist_returns(tuple(st.session_state.watchlist))
        if wl_data:
            card("?“Š ê´€?¬ì¢…ëª??˜ìµë¥??œìœ„", "1???˜ìµë¥?ê¸°ì?")
        ...
        def get_sector_returns(sectors_str):
            ...
            for sector_name, tickers_list in sectors.items():
                ...
                if sector_rets:
                    ...
                    result.append({"?¹í„°": sector_name, "?‰ê·  ?˜ìµë¥?: round(avg_ret, 2), "_ret": avg_ret})
            return sorted(result, key=lambda x: x["_ret"], reverse=True)

        sector_data = get_sector_returns(str(st.session_state.sectors))
        if sector_data:
            card("?“‚ ?¹í„°ë³??˜ìµë¥?ë¹„êµ", "1???˜ìµë¥?ê¸°ì?")
            fig_sector = go.Figure(go.Bar(
                x=[d["?¹í„°"] for d in sector_data],
                y=[d["?‰ê·  ?˜ìµë¥?] for d in sector_data],
                marker=dict(color=[CANDLE_UP if d["_ret"] >= 0 else CANDLE_DOWN for d in sector_data], opacity=0.85),
                text=[f"{d['?‰ê·  ?˜ìµë¥?]:+.1f}%" for d in sector_data],
    ...
    if st.session_state.watchlist and len(st.session_state.watchlist) >= 2:
        ...
        if corr is not None:
            card("?”— ê´€?¬ì¢…ëª??ê?ê´€ê³?, "1??ê°€ê¹Œìš¸?˜ë¡ ê°™ì´ ?€ì§ì„ Â· 0??ê°€ê¹Œìš¸?˜ë¡ ?…ë¦½??)
